const mesureService = require('../services/mesure.service');

module.exports = {
    createMesure: (req, res) => {
        mesure = req.body;
        mesureService.createMesure(mesure, (error, result) => {
            if (error) {
    ...
    updateMesure: (req, res) => {
        mesure = req.body;
        mesureService.updateMesure(mesure, (error, result) => {
            if (error) {
    ...
    getAllMesure: (req, res) => {
        id = req.query.id;
        mesureService.getAllMesure(id, (error, result) => {
            if (error) {
                ...
                console.log("Mesure recupérer avec succès");
                res.status(200).json({message : "Mesure recupérer avec succès ", result,})
            }
        });
    },
};
